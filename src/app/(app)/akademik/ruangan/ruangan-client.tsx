...
type ColumnKey = "name" | "type" | "capacity";
...
export function RuanganClient({
  ...
  const typeOptions = useMemo(() => {
    ...
    return [...types].sort((a, b) => a.localeCompare(b, "id"));
  }, [rooms]);
  ...
  }, [rooms, query, filterType]);

  const sorted = useMemo(() => {
    const getVal = (item: Room): string | number => {
      switch (sortColumn) {
        case "name":
          return item.name;
        case "type":
  ...
  const hasActiveFilters = Boolean(filterType);
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="font-heading text-2xl font-semibold tracking-[-.06em] text-[#183d32]">Ruangan</h1>
      ...
      <Card className="border-[#e2ece5] shadow-[0_3px_7px_#1c443305]">
        <CardHeader className="gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <CardTitle className="font-heading text-[#21483b]">Daftar Ruangan</CardTitle>
            <CardDescription className="text-[#8b9f95]">
              {sorted.length} dari {rooms.length} ruangan
            </CardDescription>
          </div>
        ...
        {canManage && isFilterOpen ? (
          <div className="mx-6 mb-5 rounded-[12px] border border-[#e2ece5] bg-[#f7fbf8] p-5">
            ...
            <div className="mt-4">
              <label className="text-[10px] font-bold text-[#4c6a5e]">Tipe</label>
              <select
                value={filterType}
                onChange={(event) => {
                    ...
                    >
                      {query || hasActiveFilters ? (
                        <p className="text-sm text-[#a0afa8]">
                          Tidak ada ruangan yang cocok dengan pencarian atau filter.
                        </p>
                      ) : (
                        <div className="py-6">
                          <p className="text-sm font-medium text-[#3e5c50]">Belum ada ruangan</p>
                          <p className="text-sm text-[#a0afa8]">
                            {canManage
                              ? "Tambahkan ruangan pertama untuk mulai."
                          ...
                          case "type":
                            return (
                              <TableCell key={col.key} className="px-3.5 py-3 align-middle">
                                {item.type ? (
                                  <span className="text-xs text-[#3e5c50]">{item.type}</span>
                                ) : (
            ...
            <div className="flex flex-wrap items-center justify-between gap-3 border-t border-[#f0f5f1] px-6 py-3">
              <div className="flex items-center gap-3">
                <span className="text-xs text-[#8b9f95]">
                  Menampilkan {rangeStart}–{rangeEnd} dari {sorted.length} ruangan
                </span>
                <div className="flex items-center gap-1.5">
                  <label htmlFor="ruangan-page-size" className="text-[10px] font-bold text-[#6c8279]">
                    Baris
                  </label>
                  <select
                    id="ruangan-page-size"
                    value={pageSize}
                    onChange={(event) => {
      ...
      <DetailDialog ruangan={viewing} onClose={() => setViewing(null)} />
...
function DetailDialog({
  ...
  return (
    <Dialog open={Boolean(ruangan)} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="flex max-h-[min(92vh,900px)] flex-col gap-0 overflow-hidden border-0 ring-1 ring-[#dbe8df] sm:max-w-[600px] rounded-[17px] bg-[#fbfdfb] p-0 shadow-[0_24px_70px_rgb(13_50_35/22%)]">
        ...
        {ruangan ? (
          ...
          >
            <div className="min-w-0">
              <p className="truncate text-sm font-bold text-[#2b493e]" title={ruangan.name}>
                {ruangan.name}
              </p>
            </div>
            <h3 className="mt-6 font-heading text-[14px] tracking-[-.03em] text-[#24483b]">Informasi Ruangan</h3>
            <dl className="mt-3 grid grid-cols-2 gap-x-5 gap-y-4 sm:grid-cols-3">
              <DetailRow label="Nama Ruangan" value={ruangan.name} />
              <DetailRow label="Tipe" value={ruangan.type} />
              <DetailRow label="Kapasitas" value={ruangan.capacity ?? null} />
            </dl>
          </div>
        ) : null}
      </DialogContent>
        ...
        <form action={formAction} className="flex h-full max-h-[min(92vh,900px)] flex-col">
          <DialogHeader className="shrink-0 border-b border-[#e5eee8] bg-white px-7 pb-5 pt-6">
            ...
            <DialogTitle
              ...
              style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}
            >
              {isEdit ? "Ubah Ruangan" : "Tambah Ruangan"}
            </DialogTitle>
            <DialogDescription className="mt-[7px] text-[11px] text-[#83988e]">
              Lengkapi informasi ruangan untuk menyimpan data.
            </DialogDescription>
          </DialogHeader>
          ...
          >
            ...
            <div className="space-y-2">
              <FieldLabel htmlFor="type" optional>
                Tipe
              </FieldLabel>
              <Input
                id="type"
                ...
                placeholder="Kelas / Laboratorium"
              />
            </div>
          ...
          <DialogFooter className="rounded-none border-t border-[#e3ece6] bg-white p-0 px-7 py-[24px]">
            <div className="flex w-full justify-end gap-2">
              ...
              >
                {isSubmitting ? "Menyimpan..." : isEdit ? "Simpan Perubahan" : "Simpan Ruangan"}
              </Button>
            </div>
          </DialogFooter>
        </form>
